...
#define TEST_DEVICE_NAME            "SimulaCare-RCP"
...
void app_gatt_server()
{
    esp_err_t ret;

    printf("Teste BLE!! GATT-SERVER\n");

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
    	ESP_LOGI(GATTS_TAG, "%s initialize controller failed\n", __func__);
        return;
    }
    ret = esp_bt_controller_enable(ESP_BT_MODE_BTDM);
    if (ret) {
    	ESP_LOGI(GATTS_TAG, "%s enable controller failed\n", __func__);
        return;
    }
    ret = esp_bluedroid_init();
    if (ret) {
    	ESP_LOGI(GATTS_TAG, "%s init bluetooth failed\n", __func__);
        return;
    }
    ret = esp_bluedroid_enable();
    if (ret) {
    	ESP_LOGI(GATTS_TAG, "%s enable bluetooth failed\n", __func__);
        return;
    }
    ...
    if (ret){
    	ESP_LOGI(GATTS_TAG, "gap register error, error code = %x", ret);
        return;
    }
    ret = esp_ble_gatts_app_register(PROFILE_A_APP_ID);
    if (ret){
    	ESP_LOGI(GATTS_TAG, "gatts app register error, error code = %x", ret);
        return;
    }
    ...
    if (local_mtu_ret){
    	ESP_LOGI(GATTS_TAG, "set local  MTU failed, error code = %x", local_mtu_ret);
    }
